...
plotVirusCov <- function(IP_BAM, INPUT_BAM,geneModel,libraryType = "opposite",center = mean,annotation, ZoomIn = NULL, hideStrand = "none"){
  ...
  if(hideStrand == "none"){
    ...
    p3 <- "geom_line(aes(y=(INPUT.neg),colour = \"-\"))+geom_ribbon(aes(ymax = -0.06*yscale,ymin=(IP.neg),fill = \"-\"), alpha = 0.4) +labs(y=\"normalized coverage\")+scale_x_continuous(breaks = round(seq(min(cov.data$genome_location), max(cov.data$genome_location), by = ((max(cov.data$genome_location)-min(cov.data$genome_location))/10) ) ),expand = c(0,0) )+theme_bw() + theme(panel.border = element_blank(), panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.line = element_line(colour = \"black\") )+scale_y_continuous(expand = c(0, 0))+scale_fill_discrete(name=\"IP\") + scale_colour_discrete(name=\"INPUT\")"
  }else if(hideStrand == "-"){
...
#' @param hideStrand The strand to hide when plot. Default is none.
#' @export
plotVirusCovPairs <- function(IP_BAM, INPUT_BAM, X , geneModel,libraryType = "opposite",center = mean,annotation, ZoomIn = NULL, hideStrand = "none"){
  #########################
  ### Generage gene model #
  #########################
  ## convert annotation into GRange
  ...
  ## get average from replicates
  #X <- c(rep("IP",length(IP_BAM)),rep("INPUT",length(INPUT_BAM)))
  XX <- factor(paste(c(X,X),c(rep("IP",length(IP_BAM)),rep("INPUT",length(INPUT_BAM))),sep="_" ),
               levels = unique(paste(c(X,X),c(rep("IP",length(IP_BAM)),rep("INPUT",length(INPUT_BAM))),sep="_" )) )
  
  mean_norm_positive <- data.frame( t( apply(cov.data_positive.norm,1,tapply,XX,mean) ) )
  mean_norm_negative <- data.frame( t( apply(cov.data_Negative.norm,1,tapply,XX,mean) ) )
  
  
  mean_norm_positive_1 <- mean_norm_positive[,grep(unique(X)[1],colnames(mean_norm_positive))]
  mean_norm_positive_2 <-mean_norm_positive[,-c(grep(unique(X)[1],colnames(mean_norm_positive)) )]
  
  mean_norm_negative_1 <- mean_norm_negative[,grep(unique(X)[1],colnames(mean_norm_negative))]
  mean_norm_negative_2 <-mean_norm_negative[,-c(grep(unique(X)[1],colnames(mean_norm_negative)) )]
  ...
  if(hideStrand == "none"){
    p3 <- "geom_line(aes(y=INPUT,colour = Group))+geom_ribbon(aes(ymax = IP,ymin=0,fill = Group ), alpha = 0.4)+geom_line(aes(y=(-INPUT.neg-0.06*yscale),colour = Group))+geom_ribbon(aes(ymax = -0.06*yscale,ymin=(-IP.neg-0.06*yscale),fill = Group), alpha = 0.4) +labs(y=\"normalized coverage\")+scale_x_continuous(breaks = round(seq(min(cov.data$genome_location), max(cov.data$genome_location), by = ((max(cov.data$genome_location)-min(cov.data$genome_location))/10) ) ),expand = c(0,0) )+theme_bw() + theme(panel.border = element_blank(), panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.line = element_line(colour = \"black\") )+scale_y_continuous(expand = c(0, 0))+scale_fill_discrete(name=\"IP\") + scale_colour_discrete(name=\"INPUT\")"
  }else if(hideStrand == "+"){
    yscale <-  max(mean_norm_negative)
    p3 <- "geom_line(aes(y=(-INPUT.neg-0.06*yscale),colour = Group))+geom_ribbon(aes(ymax = -0.06*yscale,ymin=(-IP.neg-0.06*yscale),fill = Group), alpha = 0.4) +labs(y=\"normalized coverage\")+scale_x_continuous(breaks = round(seq(min(cov.data$genome_location), max(cov.data$genome_location), by = ((max(cov.data$genome_location)-min(cov.data$genome_location))/10) ) ),expand = c(0,0) )+theme_bw() + theme(panel.border = element_blank(), panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.line = element_line(colour = \"black\") )+scale_y_continuous(expand = c(0, 0))+scale_fill_discrete(name=\"IP\") + scale_colour_discrete(name=\"INPUT\")"
  }else if(hideStrand == "-"){
    yscale <-  max(mean_norm_positive)
    p3 <- "geom_line(aes(y=INPUT,colour = Group))+geom_ribbon(aes(ymax = IP,ymin=0,fill = Group), alpha = 0.4)+labs(y=\"normalized coverage\")+scale_x_continuous(breaks = round(seq(min(cov.data$genome_location), max(cov.data$genome_location), by = ((max(cov.data$genome_location)-min(cov.data$genome_location))/10) ) ),expand = c(0,0) )+theme_bw() + theme(panel.border = element_blank(), panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.line = element_line(colour = \"black\") )+scale_y_continuous(expand = c(0, 0))+scale_fill_discrete(name=\"IP\") + scale_colour_discrete(name=\"INPUT\")"
  }else{ stop("Please specify strand to be hide as \"+\", \"-\" or \"none\" !! ")}
